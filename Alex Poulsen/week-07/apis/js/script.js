const getISSLocation = function() {
  ...
  .done( function(data) {
    $.ajax(`https://api.openweathermap.org/data/2.5/weather?lat=${data.iss_position.latitude}&lon=${data.iss_position.longitude}&appid=&units=metric`).done( function( weatherData ) {
      console.log( weatherData );
      $('#weather').html(`${weatherData.main.temp} degrees Celcius, ${weatherData.weather[0].description}`)
    });
  })
  .done( function(data) {
    $.ajax(`https://api.nasa.gov/planetary/earth/imagery?lon=${data.iss_position.longitude}&lat=${data.iss_position.latitude}&date=2019-11-01&api_key=`).done( function( locationData ) {
